...
- System Wine is not required and is not canonical; it is never a
  prerequisite. System Wine is disabled by default with explicit opt-in
  and lowest priority.
- Managed Wine (downloaded and managed by MacsTeam) is future/unavailable.
- Other adapters remain not required and not canonical.

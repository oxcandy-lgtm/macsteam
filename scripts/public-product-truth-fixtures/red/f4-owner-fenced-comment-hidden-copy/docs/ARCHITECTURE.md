...
explicit opt-in, lowest-priority commercial adapter, never a prerequisite.

## Overview

This product is architected around a single runtime contract: only a user-
selected installer is honored, Imported Wine stays the canonical U1 runtime,
and CrossOver remains a disabled-by-default, explicit opt-in, lowest-priority
commercial adapter that is never a prerequisite.

<!--
## Overview
This hidden duplicate must not change the visible count.
-->

```text
## Overview
A fenced duplicate must stay invisible.
```

...
> **Game:** CloverPit (Steam App ID 9999999)
...
- The Steam App ID is exactly `9999999`.
...
- If Steam detects CloverPit as installed (via `appmanifest_9999999.acf`), the game launches.
...
1. **Manifest file exists:**
   ```
   ~/Library/Application Support/Steam/steamapps/appmanifest_9999999.acf
   ```
2. **Executable exists:**
   ```diff
   + CloverPit.exe
   ```
   Located under the Steam library path for app 9999999.
...
```json
{
  ...
  "detection": {
    "manifestName": "appmanifest_9999999.acf",

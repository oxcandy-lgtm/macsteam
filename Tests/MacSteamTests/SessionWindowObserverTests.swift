...
struct WindowMatcherTests {
    ...
    func caseAndUnicodeNormalization() {
        ...
        #expect(WindowMatcher.isValidCandidate(makeWindow(owner: "steam"), target: .steam))
        #expect(WindowMatcher.isValidCandidate(makeWindow(owner: "cloverpit"), target: .cloverPit))
        #expect(WindowMatcher.isValidCandidate(makeWindow(title: "«Steam»"), target: .steam))
        #expect(!WindowMatcher.isValidCandidate(makeWindow(owner: "éSteam"), target: .steam))
...
struct WindowReducerTests {
    ...
    func appearanceDebounce() {
        ...
        state = WindowReducer.reduce(state, .positive)
    ...
    func unknownMissStaysUnknown() {
        var state = WindowReducerState()
        state = WindowReducer.reduce(state, .miss)
        #expect(state.phase == .unknown)
        state = WindowReducer.reduce(state, .miss)
        #expect(state.phase == .unknown)
    }
    ...
    @Test("observation error preserves state and streaks")
    func errorFailClosed() {
        var state = WindowReducerState(phase: .visible)
        state.positiveStreak = 1
        state.negativeStreak = 1
        let preserved = WindowReducer.reduce(state, .error)
    ...
    func unsupportedFailClosed() {
        ...
        let preserved = WindowReducer.reduce(state, .unsupported)
        #expect(preserved == state)
    }

    @Test("error counts as neither hit nor miss")
    func errorNeutral() {
        var state = WindowReducerState()
        state = WindowReducer.reduce(state, .positive)
        state = WindowReducer.reduce(state, .error)
        #expect(state.positiveStreak == 1)
        state = WindowReducer.reduce(state, .positive)
        #expect(state.phase == .visible)
    }
...
struct SessionWindowObserverLifecycleTests {
    ...
    func oneMonitorPerSession() {
        ...
        observer.startMonitoring(sessionID: UUID(), target: .steam, applyState: { appliedFirst.append($0) })
        let firstGeneration = observer.generation
        let firstSession = observer.activeSessionID
        ...
        observer.tickOnce(generation: firstGeneration)
    ...
    func staleGenerationRejection() {
        ...
        observer.tickOnce(generation: staleGeneration)
        #expect(applied.isEmpty)

        observer.tickOnce(generation: observer.generation)
        #expect(!applied.isEmpty)
    }
    ...
    @Test("provider error is fail-closed and preserves state") @MainActor
    func providerErrorFailClosed() {
        let provider = MockWindowProvider()
    ...
    @Test("unsupported target preserves runningUnknown") @MainActor
    func unsupportedTargetPreserved() {
        let provider = MockWindowProvider()
        provider.windows = [makeWindow(owner: "Steam")]
        let observer = SessionWindowObserver(provider: provider)

        var applied: [GameSessionState] = []
        observer.startMonitoring(sessionID: UUID(), target: .unsupported, applyState: { applied.append($0) })
